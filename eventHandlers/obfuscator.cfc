				...
				<script type="text/javascript">
					...
					(function($) {/*EMOTIONS:BOS*/
						...
						function #request.jsInit#(s) {
							...
							setTimeout(function() {
								<!--- get all nodes, that contain the value of "request.jsIdentifier" --->
								var $e=$(s).find(":not(iframe, script)").contents().filter(function() {
									return this.nodeType == 3 && this.textContent.indexOf('{#request.jsIdentifier#}') > -1;
								}).parent();
